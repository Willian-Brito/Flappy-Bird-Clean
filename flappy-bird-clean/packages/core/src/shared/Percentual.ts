...
export interface PerrcentualProps {
    valor?: number
    atributo?: string
    min?: number
    max?: number,
...
export default class Percentual extends ObjetoValor<Percentual> {
    constructor(props: PerrcentualProps){
        super({
            ...props,
        ...
        if(valor < min! || valor > max!) throw new Error(`O valor de ${atributo} deve ser entre ${min} e ${max}`);
    }
